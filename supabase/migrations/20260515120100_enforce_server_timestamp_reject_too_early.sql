-- Update trigger enforce_server_timestamp untuk reject insert "too early".
-- 
-- Untuk insert non-admin (mobile/anon), kalau check-in time < earliest_window
-- (jam_masuk - awal_absen_menit), tolak dengan exception jelas.
-- Pesan diawali "TOO_EARLY|HH:MM|HH:MM" agar mobile bisa parse precise
-- dan tampilkan dialog ramah ke user.
-- 
-- Hanya enforce kalau awal_absen_menit > 0 (fitur aktif untuk divisi tsb).
-- Admin (role 1-4) tetap bebas (untuk koreksi retroaktif via web).
